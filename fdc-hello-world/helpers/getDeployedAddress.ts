 ...
 * @param contractId - "ModuleName#ContractName" 形式（例: "FdcPaymentHelloWorldModule#FdcPaymentHelloWorld"）
 * @param chainId - チェーンID（例: 114 = Coston2, 19 = Songbird）
 * @param projectRoot - プロジェクトルート（デフォルト: process.cwd()）
 * @returns コントラクトアドレス（0x プレフィックス付き）
 *
 * @example
 * const address = getDeployedAddress("FdcPaymentHelloWorldModule#FdcPaymentHelloWorld", 114);
 */
